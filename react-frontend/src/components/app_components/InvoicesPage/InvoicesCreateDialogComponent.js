...
const [itemId, setItemId] = useState([])
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [companyId,itemId], setError);
        }
        set_entity({...init});
        setError({});
    }, [props.show]);
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            companyId: _entity?.companyId?._id,itemId: _entity?.itemId?._id,subTotal: _entity?.subTotal,discount: _entity?.discount,total: _entity?.total,
